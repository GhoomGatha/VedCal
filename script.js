...
function preprocess(exprRaw){
  ...
  // handle patterns like 200+50% or 200-10% meaning relative percent of the first number
  expr = expr.replace(/([0-9]+(?:\.[0-9]+)?)\s*([\+\-])\s*([0-9]+(?:\.[0-9]+)?)%/g, function(_,a,op,b){
    if(op === '+') return '('+a+' + ('+a+'*'+b+'/100))';
    if(op === '-') return '('+a+' - ('+a+'*'+b+'/100))';
    return _;
  });
  // Remaining standalone percentages: 50% -> 0.5
